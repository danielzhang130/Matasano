...
public:
    ...
    _sha1(const char *text = NULL): i(0), n_bits(0){
        state[0] = 0x67452301;
        state[1] = 0xEFCDAB89;
        state[2] = 0x98BADCFE;
        state[3] = 0x10325476;
    ...
    _sha1& add(const void *data, uint32_t n){
        if (!data) return *this;
    ...
    const _sha1& print_base64(char *base64, bool zero_terminate = true) const {
...
vector<int> sha1(const vector<int>& _message, const uint32_t _state[5], uint64_t ml){
    char* message = (char*) malloc(sizeof(char) * _message.size());
    for(size_t i = 0; i < _message.size(); ++i){
        message[i] = _message[i];
    }
	_sha1 sha;
    if(_state){
        sha.state[0] = _state[0];
        sha.state[1] = _state[1];
        sha.state[2] = _state[2];
        sha.state[3] = _state[3];
        sha.state[4] = _state[4];
    }
    if(ml){
        sha.n_bits = ml * 8;
    }
    sha.add(message, _message.size())
        .finalize();
    free(message);
    vector<int> hash;
    for(size_t i = 0; i < 20; ++i){
        hash.push_back((sha.state[i / 4] >> (24 - (i % 4) * 8)) & 255);
    }
    return hash;
}
