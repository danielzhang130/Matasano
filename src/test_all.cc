...
TEST(SET4, CTR_RANDOM_ACCESS_RW){
    using namespace ctr_random_access_rw;
    decrypt();
}

TEST(SET4, CTR_BITFLIP){
    using namespace ctr_bitflip;
    ASSERT_TRUE(bitflip());
    std::cout << "admin=true" << std::endl;
}

TEST(SET4, CBC_IV_KEY){
    using namespace cbc_iv_key;
    ASSERT_TRUE(break_key());
    std::cout << "true" << std::endl;
}

TEST(SET4, KEYED_MAC){
    using namespace keyed_mac;
    ASSERT_TRUE(length_extension());
}
